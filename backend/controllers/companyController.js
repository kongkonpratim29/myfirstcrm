...
const createCompany = async (req, res) => {
  try {
    const { name, industry, website, address, phone, email, employeeCount, gstNumber, gstVerified, gstLegalName, gstAddress } = req.body;

    const [result] = await pool.query(
      `INSERT INTO companies (name, industry, website, address, phone, email, employeeCount, gstNumber, gstVerified, gstLegalName, gstAddress, createdBy)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, industry, website, address, phone, email, employeeCount, gstNumber || null, gstVerified || false, gstLegalName || null, gstAddress || null, req.user.id]
    );
...
const updateCompany = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      `UPDATE companies 
       SET name = ?, industry = ?, website = ?, address = ?, phone = ?, email = ?, employeeCount = ?, gstNumber = ?, gstVerified = ?, gstLegalName = ?, gstAddress = ?
       WHERE id = ?`,
      [name, industry, website, address, phone, email, employeeCount, gstNumber || null, gstVerified || false, gstLegalName || null, gstAddress || null, req.params.id]
    );
